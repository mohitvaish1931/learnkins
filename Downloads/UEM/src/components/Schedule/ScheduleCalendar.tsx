    ...
    try {
      ...
      rawScheduleData.forEach((schedule, index) => {
        try {
          ...
          // Validate logical order - allow overnight trips by rolling arrival to next day
          if (arrDate <= depDate) {
            console.warn(`Schedule ${index} has arrival before or equal to departure; adjusting arrival to next day:`, {
              departure: depDate,
              arrivalBeforeAdjust: arrDate
            });
            // Treat arrival as next-day arrival (overnight trips)
            arrDate.setDate(arrDate.getDate() + 1);
            console.log(`Schedule ${index} arrival adjusted to:`, arrDate);
          }
